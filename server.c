...
{
    return ((want_right < chit->rights) && !fnmatch(chit->path, want_path, 0));
}


static int authorized_limits(client_t *cl, int sz)
{
    int low = dfs_db_nni("SELECT left FROM limits WHERE "
			 "pids LIKE ('%%'||',%d,'||'%%') "
			 "ORDER BY left LIMIT 1", cl->pid);
    if ((low >= 0) && (low < sz))
	return 0;
    
    dfs_db_do("UPDATE limits SET left=left-%d WHERE pids LIKE('%%'||',%d,'||'%%')",
	      sz, cl->pid);
    return 1;
}
...
static void server_open(Message *msg)
{
    ...
    if (!authorized_limits(cl, reply.len)) {
	sqlite3_finalize(stmt);
	goto out;
    }
...
static void server_create(Message *msg)
{
    ...
    *name++ = 0;
    ...
    free(dname);
    reply.mode = dfs_db_nni("SELECT strftime('%%s','now', 'localtime')");
    dfs_msg_send(MESSAGE_TYPE__MSG_CREATE_REPLY, client_socket, &reply);
}
...
static void handle_auth1(Message *msg)
{
    ...
    dfs_start_time(MESSAGE_TYPE__TIME_SERV_AUTH1);
    ...
    cl->skey = dfs_rsa_decrypt(&len, msg->auth1->skey.data, msg->auth1->skey.len,
			       keypri, keyprilen);
    if (!cl->skey) {
    out:
	deleteclient(cl);
    ...
    cl->chit_s = dfs_aes_decrypt(cl->skey, NULL, 
				 msg->auth1->chit.data, msg->auth1->chit.len);
    ...
    cl->nonce_c = dfs_aes_decrypt(cl->skey, NULL,
				  msg->auth1->nonce_c.data, msg->auth1->nonce_c.len);
    ...
    reply.has_nonce = 1;
    if (chit_verify(cl->chit, keypriascii, &chal_pub_key, NULL, 1, cl->pid))
	goto out;
	...
	reply.nonce.len = len;
	dfs_msg_send(MESSAGE_TYPE__MSG_AUTH1_PUB_REPLY, client_socket, &reply);
	cl->nonce_s2 = nonce_s2;
	free(buf);
	dfs_end_time(MESSAGE_TYPE__TIME_SERV_AUTH1);
    } else {
...
static void handle_auth2(Message *msg)
{
    ...
    dfs_start_time(MESSAGE_TYPE__TIME_SERV_AUTH2);
    ...
    dfs_end_time(MESSAGE_TYPE__TIME_SERV_AUTH2);
}
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "dek:l:p:Pr:s:t")) != -1) {
	switch (c) {
	case 'k':
	    key = optarg;
	    break;
